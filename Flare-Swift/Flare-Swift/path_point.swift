...
class StraightPathPoint: PathPoint {
    ...
    required init(ofType: PointType = .Straight) {
        type = ofType
    }
...
class CubicPathPoint: PathPoint {
    var type: PointType
    var translation = Vec2D()
    var weights: [Float32]?
    ...
    required init(ofType: PointType) {
        self.type = ofType
    }
    
    convenience init(fromValues t: Vec2D, _ inPoint: Vec2D, _ outPoint: Vec2D) {
